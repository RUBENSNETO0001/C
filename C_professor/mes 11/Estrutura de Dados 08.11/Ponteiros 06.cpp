...
int main (void) {
	...
	int **vet = (int **) malloc(tam * sizeof(int *)); //alocação global
	...
	//Atribuiçao dos elementos de vet
	...
	//Liberando espaço em memória de cada elemento endereçado
	for (i = 0; i < tam; i++) {
		free(vet[i]);
	}
	
	//Libera o espaço de memória global do vetor de ponteiros
	free(vet);
